...
def inject_post_into_index(post_id, post_js_entry, post_card_html):
    ...
    # Find "const posts = {" and inject right after the opening brace
    posts_pattern = r"(const posts\s*=\s*\{)"
    replacement = r"\1\n" + post_js_entry
    new_html = re.sub(posts_pattern, replacement, html, count=1)
    if new_html == html:
    ...
    # Look for the blog posts grid section and prepend the new card
    blog_grid_pattern = r'(id="blog-latest"[^>]*>)'
    card_replacement  = r"\1\n" + post_card_html
    new_html = re.sub(blog_grid_pattern, card_replacement, html, count=1)
    if new_html != html:
        html = new_html
    else:
        # Fallback: find the first post card and prepend before it
        first_card = html.find('<div class="post-card"')
        if first_card != -1:
